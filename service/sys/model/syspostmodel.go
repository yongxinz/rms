...
func NewSysPostModel(conn sqlx.SqlConn, c cache.CacheConf) SysPostModel {
	return &customSysPostModel{
		defaultSysPostModel: newSysPostModel(conn, c),
	}
}
...
func (m *customSysPostModel) Count(ctx context.Context) (int64, error) {
	var count int64
	query := fmt.Sprintf("select count(post_id) from %s", m.table)
...
func (m *customSysPostModel) DeleteMulti(ctx context.Context, postIds []int64) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf(`delete from %s where post_id in (?`+strings.Repeat(",?", len(postIds)-1)+`)`, m.table)
